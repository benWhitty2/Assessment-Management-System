...
const STAGES = ["CREATED", "RELEASED", "SUBMITTED", "WAITING_FEEDBACK", "WAITING_RESPONSE", "MARKED", "MODERATED", "COMPLETED"]; // added statges for feedback and response
...
export const ProgressPage = ({ client, user, onLogout }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                // Capture exam id and fetch exam details (for feedback/response)
                const assessmentId = data.assessment?.id;
                if (assessmentId) {
                    setExamId(assessmentId);
                    const exam = await client.getExam(assessmentId);
                    setExamData(exam);
                    setFeedback(exam.feedback || "");
                    setResponse(exam.response || "");
                }
                setLoading(false);
            } catch (error) {
    ...
    const handleStageUpdate = async (newStage) => {
        ...
        // Prevent leaving feedback/response stages without a value
        if (currentStage === "WAITING_FEEDBACK" && (!feedback || !feedback.trim())) {
            alert("Please enter feedback before progressing from this stage.");
            return;
        }
        if (currentStage === "WAITING_RESPONSE" && (!response || !response.trim())) {
            alert("Please enter a response before progressing from this stage.");
            return;
        }
    ...
    return (
        <>
            ...
            {/* Response submission box */}
            {currentStage === "WAITING_RESPONSE" && (
                <div className="feedback-card">
                    <h2>Submit Response</h2>
                    <p className="feedback-label">Feedback from marker:</p>
            ...
            {currentStage === "MARKED" && (
                <div className="feedback-card marked-card">
                    <h2>Feedback & Response</h2>
                    <p className="feedback-label">Feedback:</p>
                    <div className="feedback-display">
                        {feedback}
                    </div>
                    <p className="feedback-label">Response:</p>
                    <div className="feedback-display">
                        {response}
                    </div>
                </div>
